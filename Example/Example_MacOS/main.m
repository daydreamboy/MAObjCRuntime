...
static void TestMethodList(void)
{
    ...
    WCRTMethod *method = [methods objectAtIndex: index];
...
{
    SEL sel = @selector(description);
    WCRTMethod *method = [NSObject rt_methodForSelector: sel];
    TEST_ASSERT([method implementation] == [NSObject instanceMethodForSelector: sel]);
    TEST_ASSERT([[NSMethodSignature signatureWithObjCTypes: [[method signature] UTF8String]] isEqual: [NSObject instanceMethodSignatureForSelector: sel]]);
}
...
static void TestAddMethod(void)
{
    ...
    id obj = [[NSObject alloc] init];
    [NSObject rt_addMethod: [WCRTMethod methodWithSelector: @selector(rt_addMethodTester) implementation: (IMP)AddMethodTester signature: @"v@:"]];
...
static void TestSetMethod(void)
{
    ...
    id obj = [[NSObject alloc] init];
    WCRTMethod *method = [NSObject rt_methodForSelector: @selector(finalize)];
...
static void TestProtocolQuery(void)
{
    ...
    Protocol *protocol = [WCRTProtocol protocolWithObjCProtocol: NSProtocolFromString(@"SampleProtocol")];
    TEST_ASSERT([[protocol incorporatedProtocols] containsObject: [WCRTProtocol protocolWithObjCProtocol: NSProtocolFromString(@"NSObject")]]);
...
static void TestIvarQuery(void)
{
    ...
    WCRTIvar *ivar = [SampleClass rt_ivarForName: @"someIvar"];
...
static void TestPropertyQuery(void)
{
    ...
    WCRTProperty *property = [SampleClass rt_propertyForName: @"someProperty"];
...
{
    WCRTUnregisteredClass *unreg = [NSObject rt_createUnregisteredSubclassNamed: @"TestAddPropertyClass"];
    WCRTIvar *ivar = [WCRTIvar ivarWithName: @"assignedIvar" encode: @encode(id)];
    [unreg addIvar:ivar];
    Class c = [unreg registerClass];
    
    WCRTProperty *assignedObjectProp = [WCRTProperty propertyWithName:@"assignedObjectProp"
                                                           attributes:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                       [ivar typeEncoding], WCRTPropertyTypeEncodingAttribute,
                                                                       [ivar name], WCRTPropertyBackingIVarNameAttribute,
                                                                       nil]];
    ...
    {{
        ...
        WCRTProperty *property = [c rt_propertyForName: @"assignedObjectProp"];
        TEST_ASSERT([[property name] isEqual: @"assignedObjectProp"]);
        TEST_ASSERT([[property typeEncoding] isEqual: [NSString stringWithUTF8String: @encode(id)]]);
        TEST_ASSERT([[property ivarName] isEqual: @"assignedIvar"]);
        TEST_ASSERT([property setterSemantics] == WCRTPropertySetterSemanticsAssign);
    }}
}
#endif
...
{
    WCRTUnregisteredClass *unreg = [NSObject rt_createUnregisteredSubclassNamed: @"IvarAddTester"];
    
    [unreg addIvar: [WCRTIvar ivarWithName: @"testVar" encode: @encode(void *)]];
    Class c = [unreg registerClass];
