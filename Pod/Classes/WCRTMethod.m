...
@interface _RTObjCMethod : WCRTMethod
{
    Method _m;
}
...
@interface _RTComponentsMethod : WCRTMethod
{
    SEL _sel;
    IMP _imp;
    NSString *_sig;
...
{
    return [other isKindOfClass: [WCRTMethod class]] &&
...
@implementation NSObject (WCRTMethodSendingAdditions)

- (id)rt_sendMethod: (WCRTMethod *)method, ...
{
...
- (void)rt_returnValue: (void *)retPtr sendMethod: (WCRTMethod *)method, ...
{
    va_list args;
    va_start(args, method);
...
{
    WCRTMethod *method = [[self rt_class] rt_methodForSelector: sel];
    NSParameterAssert([[method signature] hasPrefix: [NSString stringWithUTF8String: @encode(id)]]);
